...
run-installer() {
  ...
  curl -fsSL https://raw.githubusercontent.com/Tbc159/BRadio_AzuraCast/$AZURACAST_RELEASE_BRANCH/docker-compose.installer.yml -o docker-compose.installer.yml
...
update() {
  ...
  if ask "Are you ready to continue with the update?" Y; then
    ...
    curl -fsSL https://raw.githubusercontent.com/Tbc159/BRadio_AzuraCast/$AZURACAST_RELEASE_BRANCH/docker.sh -o docker.new.sh
    ...
    if [[ ! -s docker-compose.new.yml ]]; then
      curl -fsSL https://raw.githubusercontent.com/Tbc159/BRadio_AzuraCast/$AZURACAST_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.new.yml
    fi
...
update-self() {
  ...
  curl -H 'Cache-Control: no-cache, no-store' -fsSL \
    https://raw.githubusercontent.com/Tbc159/BRadio_AzuraCast/$AZURACAST_RELEASE_BRANCH/docker.sh?$(date +%s) \
    -o docker.sh
  chmod a+x docker.sh
